...
class Fns {
	...
	public static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
		...
		if ( ! $template_path ) {
			$template_path = 'kariez-core/';
		}

		if ( ! $default_path ) {
			$default_path = untrailingslashit( KARIEZ_CORE_BASE_DIR ) . '/templates/';
		}
	...
	public static function get_assets_url( $path = null ) {
		return KARIEZ_CORE_BASE_URL . 'assets/' . $path;
	}
	...
	public static function get_post_types( $exc = '' ) {
		...
		$exclude = [ 'attachment', 'revision', 'nav_menu_item', 'elementor_library', 'tpg_builder', 'e-landing-page', 'elementor-kariez' ];
		if ( $exc ) {
