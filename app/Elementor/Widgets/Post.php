...
class Post extends ElementorBase {
	...
	public function __construct( $data = [], $args = null ) {
		$this->rt_name      = esc_html__( 'RT Post Grid', 'kariez-core' );
		$this->rt_base      = 'rt-post';
		$this->rt_translate = [
			'cols' => [
				'3'  => __( '4 Columns', 'kariez-core' ),
				'4'  => __( '3 Columns', 'kariez-core' ),
				'6'  => __( '2 Columns', 'kariez-core' ),
				'12' => __( '1 Columns', 'kariez-core' ),
			],
		];
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'default',
				'options' => [
					'default' => __( 'Post Grid 01', 'kariez-core' ),
					'grid-2' => __( 'Post Grid 02', 'kariez-core' ),
					'grid-3' => __( 'Post Grid 03', 'kariez-core' ),
					'grid-4' => __( 'Post Grid 04', 'kariez-core' ),
					'grid-5' => __( 'Post Grid 05', 'kariez-core' ),
                    'grid-6' => __( 'Post Grid 06', 'kariez-core' ),
					'list-2' => __( 'Post List 01', 'kariez-core' ),
				],
		...
		$this->add_control(
			...
			[
				'label'        => __( 'Grid Column', 'kariez-core' ),
				'type'         => \Elementor\Controls_Manager::POPOVER_TOGGLE,
				'label_off'    => __( 'Default', 'kariez-core' ),
				'label_on'     => __( 'Custom', 'kariez-core' ),
				'return_value' => 'yes',
                'condition' => [
                    'layout!' => [ 'grid-6' ]
                ],
		...
		$this->add_control(
			'gird_column_desktop',
			[
				'label'   => esc_html__( 'Grid Column for Desktop', 'kariez-core' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '4',
		...
		$this->add_control(
			'gird_column_tab',
			[
				'label'   => esc_html__( 'Grid Column for Tab', 'kariez-core' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '6',
				'options' => $this->rt_translate['cols'],
		...
		$this->add_control(
			'gird_column_mobile',
			[
				'label'   => esc_html__( 'Grid Column for Mobile', 'kariez-core' ),
				'type'    => Controls_Manager::SELECT,
				'default' => '12',
				'options' => $this->rt_translate['cols'],
        ...
        $this->add_control(
            ...
            [
                'type'                 => 'rt-select2',
                'label'                => esc_html__('Categories', 'kariez-core'),
...
//		);
        ...
        $this->add_control(
            ...
            [
                'type'        => Controls_Manager::SELECT,
                'label'   => esc_html__( 'Left Right Order', 'kariez-core' ),
                'options' => array(
                    'left-order' 		=> esc_html__( 'Left Order', 'kariez-core' ),
                    'right-order' 		=> esc_html__( 'Right Order', 'kariez-core' ),
                ),
                'default' => 'left-order',
                'condition'   => array( 'layout' => array( 'grid-6') ),
            ]
        );
		...
		$this->add_control(
			'meta_spacing',
			[
				'label'              => __( 'Meta Spacing', 'kariez-core' ),
				'type'               => Controls_Manager::DIMENSIONS,
				'size_units'         => [ 'px' ],
				'selectors'          => [
		...
		$this->add_responsive_control(
			'box_padding',
			[
				'label'              => __( 'Item Wrap Padding', 'kariez-core' ),
				'type'               => Controls_Manager::DIMENSIONS,
				'size_units'         => [ 'px' ],
				'selectors'          => [
	...
	protected function render() {
        ...
        <div class="rt-el-post-wrapper blog-<?php echo esc_attr( $data['layout'] ) ?>">
            <?php $count = 1;  ?>
            <?php if( $data['layout'] == 'grid-6' ) { ?>
                <?php Fns::get_template( "elementor/post/$template", $data ); ?>
            <?php } else { ?>
                <?php if ( $query->have_posts() ) : ?>
                    <div class="row <?php echo esc_attr( $data['item_space'] );?> justify-content-center">
                        <?php $ade = $data['delay']; $adu = $data['duration'];
                        while ( $query->have_posts() ) : $query->the_post();
                            echo '<article class="kariez-post-card ' . esc_attr( $col_class ) . ' ' . esc_attr( $data['animation'] ) . ' ' . esc_attr( $data['animation_effect'] ) . '" data-wow-delay= ' . esc_attr( $ade ) . "ms" .' data-wow-duration=' . esc_attr( $adu ) . "ms" .'>';
