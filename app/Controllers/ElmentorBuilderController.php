...
class ElmentorBuilderController {
	...
	public static $post_type = 'elementor-kariez';
	...
	public function __construct() {
		add_action( 'init', [ $this, 'header_footer_posttype' ], 10 );
		//add_action( "save_post_{self::$post_type}", [ $this, 'save_el_kariez' ] );
	...
	public function header_footer_posttype() {
		$labels = [
			'name'               => __( 'Header & Footer Builder', 'kariez-core' ),
			'singular_name'      => __( 'Header & Footer Builder', 'kariez-core' ),
			'menu_name'          => __( 'Header & Footer Builder', 'kariez-core' ),
			'name_admin_bar'     => __( 'Header & Footer Builder', 'kariez-core' ),
			'add_new'            => __( 'Add New', 'kariez-core' ),
			'add_new_item'       => __( 'Add New Header or Footer', 'kariez-core' ),
			'new_item'           => __( 'New Template', 'kariez-core' ),
			'edit_item'          => __( 'Edit Template', 'kariez-core' ),
			'view_item'          => __( 'View Template', 'kariez-core' ),
			'all_items'          => __( 'All Templates', 'kariez-core' ),
			'search_items'       => __( 'Search Templates', 'kariez-core' ),
			'parent_item_colon'  => __( 'Parent Templates:', 'kariez-core' ),
			'not_found'          => __( 'No Templates found.', 'kariez-core' ),
			'not_found_in_trash' => __( 'No Templates found in Trash.', 'kariez-core' ),
		];
	...
	public function save_el_kariez( $post_id ) {
	...
	public function override_header() {
		require KARIEZ_CORE_BASE_DIR . 'templates/builder/header.php';
		$templates   = [];
	...
	public function override_footer() {
		require KARIEZ_CORE_BASE_DIR . 'templates/builder/footer.php';
		$templates   = [];
	...
	function hfe_render_header() {
		if ( false == apply_filters( 'kariez_header_builder_enable', true ) ) {
			return;
		}
		...
		?>
        <header id="masthead" class="site-header headroom kariez-header-builder" role="banner">
        ...
        <div id="header-search" class="header-search">
            <div class="header-search-wrap">
                ...
                <form method="get" class="header-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
                    <input type="search" value="<?php echo get_search_query(); ?>" name="s" placeholder="<?php esc_attr_e( 'Type your search........', 'kariez-core' ); ?>">
	...
	function hfe_render_footer() {
		...
		if ( false == apply_filters( 'kariez_footer_builder_enable', true ) ) {
			return;
		}
		?>
