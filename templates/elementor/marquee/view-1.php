...
<div class="rt-marquee-slider">
	<div class="rt-marquee <?php echo esc_attr( $marquee_direction );?>">
		<div class="rt-marquee-item">
			...
			?>
			<<?php echo esc_attr( $heading_tag ); ?> class="entry-title <?php if( $gradient_display == 'yes' ) { ?>title-gradient<?php } ?> <?php echo esc_attr( $animation );?> <?php echo esc_attr( $animation_effect );?>" data-per="<?php kariez_html( $item['title'], 'allow_title' );?>" data-wow-delay="<?php echo esc_attr( $ade );?>ms" data-wow-duration="<?php echo esc_attr( $adu );?>ms">
			<?php kariez_html( $title, 'allow_title' ); ?>
			<?php  if('icon' == $icon_type || 'image' == $icon_type) : ?>
				<span class="icon-holder <?php echo $spin_animations? 'spin' : ''; ?>">
			        <?php
			        if('icon' == $icon_type) {
